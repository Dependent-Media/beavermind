...
- **E2E** (`e2e.yml`) — Playwright clone-from-URL spec. Runs on every PR + push to main + manual dispatch. Before running Playwright, the workflow rsyncs the PR's plugin code to `testbeavermind` so the test exercises the PR's actual changes (not whatever was last manually deployed). Uses a global concurrency group so concurrent PRs don't race on the shared test site. Uploads video + screenshots as artifacts. Requires SSH secrets (see `docs/ci-setup.md`).
